...
def _config_payload(address: str, provider: str, content: list) -> dict:
    uuid = unique()
    return {
        'PayloadContent': content,
        'PayloadDisplayName': f'{address} ({provider})',
...
class AppleGenerator(ConfigGenerator):
    def client_config(self, local_part: str, domain_part: str, display_name: str, password: str) -> str:
        ...
        if not domain:
            domain: Domain = Domain.query.filter_by(name='DEFAULT').first()
        if not domain:
        ...
        config = _config_payload(f'{local_part}@{domain_part}', provider.short_name, config)
